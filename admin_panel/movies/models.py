...
class Movie(TimeStampedModel):
    ...
    id = models.UUIDField(
        _("идентификатор"), primary_key=True, default=uuid4, editable=False
    )
    title = models.CharField(_("название"), max_length=255)
    description = models.TextField(_("описание"), blank=True, null=True)
    creation_date = models.DateField(
        _("дата создания фильма"), blank=True, db_index=True
    )
    age_limit = models.PositiveSmallIntegerField(
        _("ограничение по возрасту"), validators=[MaxValueValidator(21)], db_index=True
    )
    rating = models.FloatField(
        _("рейтинг пользователей"),
        blank=True,
        validators=[MinValueValidator(0)],
        db_index=True,
    )
    imdb_rating = models.FloatField(
        _("IMDB рейтинг"), validators=[MinValueValidator(0)], db_index=True
    )
    movie_type = models.CharField(
        _("тип"), max_length=20, choices=MovieType.choices, db_index=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["title", "creation_date"], name="movie_unique"
            )
        ]
...
class MoviePerson(TimeStampedModel):
    ...
    movie = models.ForeignKey(
        Movie, on_delete=models.CASCADE, verbose_name=_("кинопроизведение")
    )
    person = models.ForeignKey(
        Person, on_delete=models.CASCADE, verbose_name=_("человек")
    )
    role = models.CharField(
        _("роль"), max_length=20, choices=RoleType.choices, db_index=True
    )

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["movie", "person", "role"], name="movie_person_unique"
            )
        ]
