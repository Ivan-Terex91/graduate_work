...
class StripeClient:
    ...
    async def _request(self, method: str, endpoint: str, headers: dict = None, data: dict = None):
        async with ClientSession(headers=self.auth_header) as session:
            async with session.request(
                    method=method, url=f"{self.base_url}{endpoint}", headers=headers, data=data
            ) as resp:
    ...
    async def create_customer(self, user_name: str, user_email: str):
        ...
        customer_data = {"description": "description", "email": user_email, "name": user_name}
        return await self._request(method="POST", endpoint="/customers", data=customer_data)

    async def create_payment(self, customer_id: str, amount: float, currency: str, user_email: str,
                             payment_method):
        """Создание платежа"""
        ...
        payment_intent_data = {"customer": customer_id, "amount": amount, "currency": currency,
                               "receipt_email": user_email, "payment_method": "pm_card_visa"}  # TODO payment_method
        return await self._request(method="POST", endpoint="/payment_intents", data=payment_intent_data)
    ...
    async def confirm_payment(self):
        ...
        return await self._request(method="POST", endpoint="/payment_intents/pi_3JRENwDRo0HQjSKX27nufHNv/confirm",
                                   data=data)

    async def get_payment_data(self, payment_intents_id: str):
        """Получение данных о платеже"""
        return await self._request(method="GET", endpoint=f"/payment_intents/{payment_intents_id}")
